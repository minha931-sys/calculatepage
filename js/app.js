...
date:{n:'날짜 계산기',c:'life',d:'기준 날짜에서 며칠 전후 날짜를 계산하고, 두 날짜 사이의 차이도 함께 확인합니다.',f:[['date','기준 날짜',''],['number','더할 일수','100']],op:'date',r:['d-day','age']},'d-day':{n:'디데이 계산기',c:'life',d:'중요한 날짜까지 남은 날 또는 지난 날을 계산합니다.',f:[['date','목표 날짜','']],op:'dday',r:['date','exam-dday']},age:{n:'나이 계산기',c:'life',d:'출생일을 기준으로 현재 한국식 나이를 계산합니다.',f:[['date','생년월일','']],op:'age',r:['international-age','d-day']},'international-age':{n:'만나이 계산기',c:'life',d:'출생일을 기준으로 현재 만 나이를 계산합니다.',f:[['date','생년월일','']],op:'internationalage',r:['age','date']},time:{n:'근무 시간 계산기',c:'life',d:'시작·종료 시간을 입력해 경과 시간을 계산합니다.',f:[['time','시작 시간','09:00'],['time','종료 시간','18:00']],op:'time',r:['work-hours','date']},'dutch-pay':{n:'더치페이 계산기',c:'life',d:'여러 정산 항목을 추가하고 전체 인원수로 1인당 더치페이 금액을 계산합니다.',f:[['number','총 금액(원)','50000'],['number','인원 수','4']],op:'dutch',r:['discount','budget']},unit:{n:'단위 변환 계산기',c:'life',d:'길이와 무게 단위를 빠르게 변환합니다.',f:[['number','값','1'],['select','변환','km → m|m → cm|kg → g']],op:'unit',r:['percent','time']},
 vat:{n:'부가세 계산기',c:'business',d:'공급가액 또는 합계 금액을 기준으로 부가세를 계산합니다.',f:[['number','공급가액(원)','100000']],op:'vat',r:['margin','estimate']},margin:{n:'마진율 계산기',c:'business',d:'판매가와 원가를 입력해 이익과 마진율을 계산합니다.',f:[['number','판매가(원)','50000'],['number','원가(원)','30000']],op:'margin',r:['vat','freelance-rate']},wage:{n:'시급 계산기',c:'business',d:'월급과 월 근무시간으로 예상 시급을 계산합니다.',f:[['number','월 급여(원)','2500000'],['number','월 근무시간','209']],op:'wage',r:['work-hours','salary']},'work-hours':{n:'주간 근무시간 계산기',c:'business',d:'7일간 근무 기록을 합산하고 날짜별 내역과 CSV를 만듭니다.',f:[['time','출근 시간','09:00'],['time','퇴근 시간','18:00'],['number','휴게 시간(분)','60']],op:'work',r:['wage','time']},estimate:{n:'견적 계산기',c:'business',d:'수량과 단가를 입력해 견적 금액과 부가세를 계산합니다.',f:[['number','수량','10'],['number','단가(원)','10000']],op:'estimate',r:['vat','margin']},'freelance-rate':{n:'프리랜서 단가 계산기',c:'business',d:'목표 월수입과 작업 시간을 기준으로 필요한 시간당 단가를 계산합니다.',f:[['number','목표 월수입(원)','4000000'],['number','월 작업시간','120']],op:'freelance',r:['estimate','wage']},severance:{n:'퇴직금 계산기',c:'business',d:'평균 임금과 근속일수를 기준으로 예상 퇴직금을 계산합니다.',f:[['number','최근 3개월 임금 합계(원)','9000000'],['number','근속 일수','730']],op:'severance',r:['wage','salary']}}
const cats={money:['돈 계산기','₩','돈과 금융에 필요한 계산기','percent discount savings-interest installment loan-interest salary budget'],education:['교육 계산기','A+','학점과 시험을 위한 계산기','gpa target-gpa retake school-grade average-score exam-dday'],health:['건강 계산기','BMI','건강 관리와 운동 계산기','bmi bmr calorie water exercise-calorie target-weight running-pace'],life:['생활 계산기','D+','매일 필요한 생활 계산기','date d-day age international-age time dutch-pay unit'],business:['업무 계산기','%','일과 사업에 필요한 계산기','vat margin wage work-hours estimate freelance-rate severance']};
Object.assign(calculators['exercise-calorie'],{
...
function bindStandardCalculator(){
  ...
  button.onclick=()=>{
    ...
    try{
      ...
      if(inputs.some(input=>String(input.value).trim()===''))throw new Error('필수 입력값이 비어 있습니다.');
      if(inputs.some(input=>input.type==='number'&&(!Number.isFinite(Number(input.value))||Number(input.value)<0)))throw new Error('0 이상의 숫자를 입력해 주세요.');
      const answer=compute(config.op,inputs);
      if(!answer||!answer[0]||/NaN|Infinity|undefined/.test(answer.join(' ')))throw new Error('계산 결과를 만들 수 없습니다.');
      result.innerHTML=`<strong>${answer[0]}</strong><p>${answer[1]||''}</p>`;
      result.classList.add('show');
    }catch(error){
...
function enhancePercentCalculator(){
  ...
  const root=document.querySelector('#calculator');
  root.innerHTML=`<h1>퍼센트 계산기</h1><p class="lead">전체의 일정 비율, 부분이 차지하는 비율, 기준값 대비 증가율·감소율을 계산하세요.</p><section class="calculator-box percent-box"><div class="percent-tabs" role="tablist" aria-label="퍼센트 계산 방식"><button class="percent-tab active" data-mode="of" role="tab">전체의 몇 %</button><button class="percent-tab" data-mode="whole" role="tab">전체 구하기</button><button class="percent-tab" data-mode="ratio" role="tab">몇 %인지</button><button class="percent-tab" data-mode="change" role="tab">증가·감소율</button></div><div id="percent-form"></div><div class="result" id="percent-result" aria-live="polite"></div></section><section class="content-block"><h2>퍼센트 계산 유형</h2><ol><li><strong>전체의 몇 %</strong>: 전체값 × 퍼센트 ÷ 100으로 부분값을 구합니다.</li><li><strong>부분이 몇 %인지</strong>: 부분값 ÷ 전체값 × 100으로 비율을 구합니다.</li><li><strong>증가율·감소율</strong>: (변경값 − 기준값) ÷ 기준값 × 100으로 변화를 구합니다.</li><li><strong>전체 구하기</strong>: 부분값 ÷ 퍼센트 × 100으로 원래 전체값을 역산합니다.</li></ol><p>퍼센트포인트(%p)는 두 퍼센트 수치의 단순 차이입니다. 예를 들어 20%에서 25%는 5%p 상승이지만 변화율은 25%입니다.</p></section><section class="content-block"><h2>관련 계산기</h2><div class="related"><a href="/calculators/discount.html">할인율 계산기</a><a href="/calculators/percent-change.html">퍼센트 증가율 계산기</a><a href="/calculators/vat.html">부가세 계산기</a><a href="/calculators/margin.html">마진율 계산기</a></div></section>`;
  const forms={
    of:{title:'전체의 몇 % 구하기',hint:'예: 50,000원의 20%는 10,000입니다.',fields:[['전체 값','50000'],['퍼센트(%)','20']],answer:(a,b)=>[a*b/100,'',`${a.toLocaleString('ko-KR')} × ${b} ÷ 100`]},
    whole:{title:'부분값과 비율로 전체 구하기',hint:'예: 15,000이 전체의 15%일 때 전체값은 100,000입니다.',fields:[['부분 값','15000'],['퍼센트(%)','15']],answer:(a,b)=>[a/b*100,'',`${a.toLocaleString('ko-KR')} ÷ ${b} × 100`]},
    ratio:{title:'부분값은 전체의 몇 %인지 구하기',hint:'예: 20은 80의 25%입니다.',fields:[['부분 값','20'],['전체 값','80']],suffix:'%',answer:(a,b)=>[a/b*100,'%',`${a.toLocaleString('ko-KR')} ÷ ${b.toLocaleString('ko-KR')} × 100`]},
    change:{title:'기준값 대비 증가율·감소율',hint:'예: 100에서 120으로 증가하면 변화율은 20%입니다.',fields:[['기준 값','100'],['변경 값','120']],suffix:'%',allowNegative:true,answer:(a,b)=>[(b-a)/a*100,'%',`${a.toLocaleString('ko-KR')}에서 ${b.toLocaleString('ko-KR')}으로 ${b>=a?'증가':'감소'}`]}
  };
  let mode='of';const form=root.querySelector('#percent-form'),result=root.querySelector('#percent-result');
  const render=()=>{const data=forms[mode];form.innerHTML=`<h2>${data.title}</h2><p>${data.hint}</p><div class="percent-fields">${data.fields.map((field,i)=>`<label><span>${field[0]}</span><input id="percent-value-${i}" type="number" ${data.allowNegative?'':'min="0"'} step="any" inputmode="decimal" placeholder="예: ${field[1]}"></label>`).join('')}</div><button class="primary-btn" id="calculate-percent" type="button">계산하기</button>`;form.querySelector('#calculate-percent').onclick=()=>{const raw=[form.querySelector('#percent-value-0').value,form.querySelector('#percent-value-1').value],a=Number(raw[0]),b=Number(raw[1]);const invalid=raw.some(value=>value.trim()==='')||!Number.isFinite(a)||!Number.isFinite(b)||(!data.allowNegative&&(a<0||b<0))||((mode==='whole'||mode==='ratio')&&b===0)||(mode==='change'&&a===0);if(invalid){result.innerHTML='<strong>입력값을 확인해 주세요</strong><p>빈칸과 숫자 범위를 확인하세요. 나누는 전체값·비율 또는 변화율의 기준값은 0일 수 없습니다.</p>';result.classList.add('show');return}const [value,suffix,text]=data.answer(a,b);if(!Number.isFinite(value)){result.innerHTML='<strong>계산할 수 없는 값입니다</strong><p>0으로 나누지 않았는지 입력값을 다시 확인해 주세요.</p>';result.classList.add('show');return}result.innerHTML=`<strong>${Number(value.toFixed(6)).toLocaleString('ko-KR')}${suffix}</strong><p>${text}</p>`;result.classList.add('show')}};
...
function enhanceExamDdayCalculator(){
  if(document.body.dataset.calculator!=='exam-dday')return;
  const root=document.querySelector('#calculator');
  ...
  root.querySelector('#exam-dday-calc').onclick=()=>{
    ...
    const [baseYear,baseMonth,baseDay]=baseValue.split('-').map(Number),[targetYear,targetMonth,targetDay]=targetValue.split('-').map(Number);
    const raw=(Date.UTC(targetYear,targetMonth-1,targetDay)-Date.UTC(baseYear,baseMonth-1,baseDay))/86400000,future=raw>=0;
    if(!Number.isFinite(raw)){result.innerHTML='<strong>날짜를 확인해 주세요</strong><p>유효한 기준일과 시험일을 선택해 주세요.</p>';result.classList.add('show');return}
    let days=Math.abs(raw)+(root.querySelector('#exam-include-base').checked?1:0)-(root.querySelector('#exam-include-target').checked?0:1);
    days=Math.max(0,days);
    result.innerHTML=`<div class="utility-result-grid"><div><span>시험 D-day</span><strong>${raw===0?'D-Day':future?'D-'+days:'D+'+days}</strong></div><div><span>주 단위</span><b>${Math.floor(days/7)}주 ${days%7}일</b></div><div><span>계산 구간</span><b>${baseValue} ~ ${targetValue}</b></div></div><p>${raw===0?'기준일과 시험일이 같습니다.':future?'남은 공부 기간입니다.':'시험일로부터 지난 기간입니다.'} 자정 전후에는 기준 날짜가 달라질 수 있습니다.</p>`;
    result.classList.add('show');
  };
}
enhanceExamDdayCalculator();
...
function addSavingsTaxOption(){
  ...
  button.onclick=()=>{
    ...
    if(!Number.isFinite(amount)||!Number.isFinite(rate)||!Number.isFinite(months)||amount<=0||rate<0||months<=0||!Number.isInteger(months)){result.innerHTML='<strong>입력값을 확인해 주세요</strong><p>금액은 0보다 크게, 금리는 0 이상, 기간은 1개월 이상의 정수로 입력해 주세요.</p>';result.classList.add('show');return}
    ...
    const interest=installment?amount*rate/100*months*(months+1)/24:amount*rate/100*months/12;
...
function enhanceEstimateCalculator(){
  ...
  const root=document.querySelector('#calculator');
  const isStatic=root.hasAttribute('data-static-rendered');
  const row=()=>`<tr><td><input class="estimate-name" type="text" aria-label="항목명" placeholder="예: 디자인 작업"></td><td><input class="estimate-qty" type="number" min="0" step="any" aria-label="수량" placeholder="예: 2"></td><td><input class="estimate-price" type="number" min="0" aria-label="단가" placeholder="예: 50000"></td><td class="estimate-line">0원</td><td><button type="button" class="row-delete" aria-label="항목 삭제">×</button></td></tr>`;
...
function enhanceUnemploymentBenefitCalculator(){
  ...
  const unemploymentTitle=root.querySelector('h1'),unemploymentLead=root.querySelector('.lead');
  ...
  root.querySelector('#calculate-ub').onclick=()=>{
    ...
    if(!Number.isFinite(salary)||salary<=0||!Number.isFinite(hours)||hours<1||hours>8||!days){
      result.innerHTML='<strong>급여 정보와 지급일수를 확인해 주세요</strong><p>평균 월급, 1일 소정근로시간(1~8시간), 직접 선택한 예상 지급일수가 필요합니다.</p>';
      result.classList.add('show');
      return;
    }
    if((start&&!end)||(!start&&end)){
      result.innerHTML='<strong>재직기간 입력을 확인해 주세요</strong><p>재직기간을 표시하려면 입사일과 퇴사일을 모두 선택하세요. 표시하지 않으려면 둘 다 비워둘 수 있습니다.</p>';
      result.classList.add('show');
      return;
    }
    const startDate=start?new Date(`${start}T00:00:00`):null,endDate=end?new Date(`${end}T00:00:00`):null;
    if(startDate&&endDate<startDate){
      result.innerHTML='<strong>재직 기간을 확인해 주세요</strong><p>퇴사일은 입사일과 같거나 뒤여야 합니다.</p>';
      result.classList.add('show');
      return;
    }
    const employmentDays=startDate?Math.round((endDate-startDate)/86400000)+1:null;
    const rawDaily=salary/30*.6,lowerDaily=10320*hours*.8,daily=Math.round(Math.min(68100,Math.max(lowerDaily,rawDaily)));
    const total=daily*days,money=v=>Math.round(v).toLocaleString('ko-KR')+'원';
    const applied=daily===68100?'2026년 상한 적용':daily===Math.round(lowerDaily)?'2026년 하한 적용':'월급 기준 산정';
    const employmentCard=employmentDays===null?'':`<div><span>참고 재직 기간</span><strong>${Math.floor(employmentDays/30)}개월</strong><small>${employmentDays.toLocaleString()}일 · 자격 판정 미반영</small></div>`;
    result.innerHTML=`<div class="unemployment-result-grid">${employmentCard}<div><span>1일 예상액</span><b>${money(daily)}</b><small>${applied}</small></div><div><span>예상 총 수령액</span><b>${money(total)}</b><small>직접 선택한 ${days}일 기준</small></div></div><p class="unemployment-summary">재직기간은 지급일수나 수급 자격 계산에 사용하지 않았습니다. 실제 자격과 지급일수는 공식 안내를 확인하세요.</p>`;
    result.classList.add('show');
  };
}
...
if(type==='day-count'){root.innerHTML=`<h1>일수 계산기</h1><p class="lead">시작일과 종료일 사이의 기간을 일수와 주 단위로 계산하세요.</p><section class="calculator-box age-box"><div class="age-fields"><label><span>시작일</span><input id="dc-start" type="date"></label><label><span>종료일</span><input id="dc-end" type="date"></label></div><label class="tax-option"><input id="dc-inclusive" type="checkbox"> 시작일과 종료일 포함</label><button class="primary-btn" id="dc-calc">일수 계산하기</button><div class="result" id="dc-result"></div></section>`;root.querySelector('#dc-calc').onclick=()=>{const s=root.querySelector('#dc-start').value,e=root.querySelector('#dc-end').value,r=root.querySelector('#dc-result');if(!s||!e){r.innerHTML='<strong>두 날짜를 모두 선택해 주세요</strong><p>시작일과 종료일을 입력하면 기간을 계산할 수 있습니다.</p>';r.classList.add('show');root.querySelector(!s?'#dc-start':'#dc-end').focus();return}let days=Math.round((new Date(e)-new Date(s))/86400000);if(days<0){r.innerHTML='<strong>날짜 순서를 확인해 주세요</strong><p>종료일은 시작일과 같거나 뒤여야 합니다.</p>';r.classList.add('show');root.querySelector('#dc-end').focus();return}if(root.querySelector('#dc-inclusive').checked)days++;r.innerHTML=`<div class="utility-result-grid"><div><span>총 기간</span><strong>${days.toLocaleString()}일</strong></div><div><span>주 단위</span><b>${Math.floor(days/7)}주 ${days%7}일</b></div><div><span>개월 환산</span><b>${(days/30.44).toFixed(1)}개월</b></div></div>`;r.classList.add('show')}}
}
...
const restoredTools={
 ...
 'compound-interest':['적립식 복리 계산기','money',['초기 투자금(원)','연 수익률(%)','기간(년)']],
 'body-fat':['체지방률 계산기','health',['키(cm)','허리둘레(cm)','몸무게(kg)']],
 'fuel-cost':['연료비 계산기','life',['주행 거리(km)','연비(km/L)','유가(원/L)']]
};
const restoredDescriptions={
  'four-insurance':'월 보수액을 기준으로 국민연금·건강보험·장기요양·고용보험의 근로자 부담액을 추정합니다.',
  'annual-salary':'계약 연봉과 별도 상여, 지급 개월 수를 반영해 월 세전·세후 평균을 비교합니다.',
  'jeonse-loan':'전세대출 원금과 연 금리, 상환 방식을 기준으로 월 이자와 총 부담을 추정합니다.',
  'parental-leave':'통상임금과 적용 비율·상하한을 직접 입력해 육아휴직 급여 예상액을 계산합니다.',
  'car-installment':'차량 가격·선수금·부대비용·금리·기간을 반영해 월 할부금과 총 이자를 계산합니다.',
  'volumetric-weight':'박스 가로·세로·높이와 운송사 분모로 부피무게를 계산합니다.',
  'compound-interest':'초기 투자금과 월 납입액, 수익률, 기간을 반영한 복리 잔액을 계산합니다.',
  'body-fat':'성별과 키·목·허리·엉덩이 둘레로 미 해군식 체지방률 참고값을 추정합니다.',
  'fuel-cost':'거리·연비·유가·반복 횟수로 주유비와 이동 비용을 계산합니다.'
};
Object.entries(restoredTools).forEach(([key,value])=>calculators[key]={n:value[0],c:value[1],d:restoredDescriptions[key]});
...
if(document.body.dataset.customCalculator==='body-fat'){
  const root=document.querySelector('#calculator');
  root.innerHTML=`<a class="calculator-home" href="/categories/health.html">건강 카테고리 더보기</a><h1>체지방률 계산기</h1><p class="lead">성별과 신체 둘레를 입력해 미 해군식 체지방률 참고값을 계산합니다.</p><section class="calculator-box utility-box"><div class="utility-form"><div class="utility-fields"><label><span>성별</span><select id="bf-sex"><option value="male">남성</option><option value="female">여성</option></select></label><label><span>키(cm)</span><input id="bf-height" type="number" min="1" step="0.1" inputmode="decimal" placeholder="예: 175"></label><label><span>목둘레(cm)</span><input id="bf-neck" type="number" min="1" step="0.1" inputmode="decimal" placeholder="예: 38"></label><label><span>허리둘레(cm)</span><input id="bf-waist" type="number" min="1" step="0.1" inputmode="decimal" placeholder="예: 85"></label><label><span>엉덩이둘레(cm, 여성만)</span><input id="bf-hip" type="number" min="1" step="0.1" inputmode="decimal" placeholder="예: 95"></label></div><button class="primary-btn" id="bf-calc" type="button">체지방률 계산하기</button></div><div class="result" id="bf-result" aria-live="polite"></div><p class="calculator-note">줄자 측정과 공식을 이용한 통계적 추정치이며 의료 진단이나 체성분 검사 결과를 대신하지 않습니다.</p></section><section class="content-block"><h2>입력과 계산 공식</h2><p>남성은 키·목·허리둘레를, 여성은 엉덩이둘레까지 사용합니다. cm를 inch로 바꾼 뒤 남성은 86.010×log10(허리-목)-70.041×log10(키)+36.76, 여성은 163.205×log10(허리+엉덩이-목)-97.684×log10(키)-78.387을 적용합니다.</p></section><section class="content-block"><h2>계산 예시와 해석</h2><p>남성, 키 175cm, 목 38cm, 허리 85cm를 입력하면 약 17.0%입니다. 측정 위치와 줄자 압력만 달라져도 값이 달라질 수 있으므로 같은 조건에서의 변화 추적에 활용하세요.</p></section><section class="content-block"><h2>관련 계산기</h2><div class="related"><a href="/calculators/bmi.html">BMI 계산기</a><a href="/calculators/target-weight.html">적정 체중 계산기</a><a href="/calculators/bmr.html">기초대사량 계산기</a><a href="/calculators/calorie.html">칼로리 계산기</a></div></section>`;
  root.querySelector('#bf-calc').onclick=()=>{
    const sex=root.querySelector('#bf-sex').value,height=Number(root.querySelector('#bf-height').value),neck=Number(root.querySelector('#bf-neck').value),waist=Number(root.querySelector('#bf-waist').value),hip=Number(root.querySelector('#bf-hip').value),result=root.querySelector('#bf-result');
    const required=[height,neck,waist,...(sex==='female'?[hip]:[])];
    if(required.some(value=>!Number.isFinite(value)||value<=0)||waist<=neck||(sex==='female'&&waist+hip<=neck)){result.innerHTML='<strong>입력값을 확인해 주세요</strong><p>필수 신체 치수는 0보다 크게 입력하고 허리둘레는 목둘레보다 크게 입력해 주세요.</p>';result.classList.add('show');return}
    const toInch=value=>value/2.54;
    const estimate=sex==='male'?86.010*Math.log10(toInch(waist)-toInch(neck))-70.041*Math.log10(toInch(height))+36.76:163.205*Math.log10(toInch(waist)+toInch(hip)-toInch(neck))-97.684*Math.log10(toInch(height))-78.387;
    if(!Number.isFinite(estimate)||estimate<0||estimate>75){result.innerHTML='<strong>측정값을 다시 확인해 주세요</strong><p>공식으로 해석하기 어려운 결과입니다. 측정 위치와 단위를 확인해 주세요.</p>';result.classList.add('show');return}
    result.innerHTML=`<strong>${estimate.toFixed(1)}%</strong><p>입력한 둘레를 미 해군식에 적용한 참고값입니다. 같은 위치와 조건에서 반복 측정한 변화량을 중심으로 보세요.</p>`;
    result.classList.add('show');
  };
}

const batchSearchTools={
 'percent-change':['퍼센트 증가율·감소율 계산기','money','이전 값과 이후 값의 차이와 증가율·감소율을 계산합니다.'],
 'break-even':['손익분기점 계산기','business','고정비·판매가·변동비로 손익분기 판매량과 목표이익 수량을 계산합니다.'],
 roi:['ROI 계산기','money','초기 투자금·추가 비용·회수금으로 ROI와 연환산 수익률을 계산합니다.'],
 exchange:['환율 계산기','money','외화 금액과 환율·수수료·우대율로 실제 원화 환산액을 추정합니다.'],
 'shipping-split':['배송비 분할 계산기','business','상품 금액이나 수량에 따라 배송비 분담액을 나눠 계산합니다.'],
 electricity:['에어컨 전기세 계산기','life','에어컨 소비전력과 사용시간으로 월 전력사용량과 간편 예상 비용을 계산합니다.'],
 'travel-budget':['여행 경비 계산기','life','교통·숙박·식비·인원수로 총 여행 예산과 1인당 비용을 계산합니다.'],
 'calorie-deficit':['칼로리 적자 계산기','health','유지 열량과 섭취 열량 차이로 일·주간 칼로리 적자를 추정합니다.'],
 'exam-target':['시험 성적 목표 계산기','education','현재 반영 점수와 목표 점수로 남은 시험에서 필요한 점수를 역산합니다.'],
 'income-tax':['근로소득 세금 간편 계산기','business','연간 총급여와 공제·기납부세액으로 소득세와 지방소득세를 추정합니다.']};
Object.entries(batchSearchTools).forEach(([key,value])=>calculators[key]={n:value[0],c:value[1],d:value[2]});
...
function makeHomeCategoriesExpandable(){
  ...
  const render=(key)=>{const data=cats[key],ids=[...new Set(data[3].trim().split(/\s+/))].filter(id=>calculators[id]).sort((a,b)=>calculators[a].n.localeCompare(calculators[b].n,'ko-KR'));panel.innerHTML=`<div class="category-panel-heading"><div><p>${data[2]}</p><h3>${data[0]}</h3></div><a href="/categories/${key}.html">전체 보기 →</a></div><div class="category-calculator-list">${ids.map(id=>`<a href="${href(id)}"><b>${calculators[id].n}</b>${calculators[id].d?`<span>${calculators[id].d}</span>`:''}</a>`).join('')}</div>`;panel.classList.add('show')};
...
if(document.body.dataset.calculator==='exercise-calorie'){
  ...
  root.querySelector('#ec-calc').onclick=()=>{const weight=+root.querySelector('#ec-weight').value,met=+root.querySelector('#ec-type').value,minutes=+root.querySelector('#ec-minutes').value,result=root.querySelector('#ec-result');if(!Number.isFinite(weight)||weight<=0||!Number.isFinite(minutes)||minutes<=0){result.innerHTML='<strong>입력값을 확인해 주세요</strong><p>몸무게와 운동 시간은 0보다 크게 입력하세요.</p>';result.classList.add('show');(!Number.isFinite(weight)||weight<=0?root.querySelector('#ec-weight'):root.querySelector('#ec-minutes')).focus();return}const calories=met*3.5*weight/200*minutes;result.innerHTML=`<strong>${Math.round(calories).toLocaleString()} kcal</strong><p>${root.querySelector('#ec-type').selectedOptions[0].text} · ${minutes}분 기준 예상 소모량입니다.</p>`;result.classList.add('show')};
}
...
function validateStandardCalculator(){
  ...
  button.onclick=()=>{
    const inputs=[...document.querySelectorAll('.fields input,.fields select')];
    const result=document.querySelector('#result');
    const firstBlank=inputs.find(input=>!String(input.value).trim()||input.value==='선택');
    const firstInvalidNumber=inputs.find(input=>input.type==='number'&&(!Number.isFinite(Number(input.value))||Number(input.value)<0));
    let message='';
    let focusTarget=firstBlank||firstInvalidNumber;
    if(firstBlank)message='모든 값과 선택 항목을 입력해 주세요.';
    else if(firstInvalidNumber)message='숫자 입력값은 0 이상으로 입력해 주세요.';
    else if(key==='wage'&&Number(inputs[1]?.value)<=0){message='월 근무시간은 0보다 크게 입력해 주세요.';focusTarget=inputs[1]}
    else if(key==='work-hours'){
      const toMinutes=value=>value.split(':').reduce((sum,part)=>sum*60+Number(part),0);
      const start=toMinutes(inputs[0].value),end=toMinutes(inputs[1].value);
      const elapsed=(end-start+1440)%1440||1440,breakMinutes=Number(inputs[2].value);
      if(breakMinutes>=elapsed){message='휴게시간은 출근부터 퇴근까지의 전체 시간보다 짧아야 합니다.';focusTarget=inputs[2]}
    }else if(key==='international-age'&&new Date(`${inputs[0].value}T00:00:00`)>new Date()){
      message='생년월일은 오늘보다 미래일 수 없습니다.';
      focusTarget=inputs[0];
    }
    if(message){result.innerHTML=`<strong>입력값을 확인해 주세요</strong><p>${message}</p>`;result.classList.add('show');focusTarget?.focus();return}
...
if(document.body.dataset.calculator==='calorie'){
  ...
  root.querySelector('#cal-calc').onclick=()=>{const bmr=+root.querySelector('#cal-bmr').value,factor=+root.querySelector('#cal-level').value,r=root.querySelector('#cal-result');if(!Number.isFinite(bmr)||bmr<=0){r.innerHTML='<strong>기초대사량을 입력해 주세요</strong><p>기초대사량은 0보다 큰 숫자로 입력하세요.</p>';r.classList.add('show');root.querySelector('#cal-bmr').focus();return}const total=bmr*factor;r.innerHTML=`<strong>${Math.round(total).toLocaleString()} kcal</strong><p>${root.querySelector('#cal-level').selectedOptions[0].text} 기준 하루 유지 칼로리입니다.</p>`;r.classList.add('show')};
}
...
if(document.body.dataset.calculator==='water'){
  ...
  root.querySelector('#water-calc').onclick=()=>{const weightText=root.querySelector('#water-weight').value.trim(),exerciseText=root.querySelector('#water-exercise').value.trim(),weight=Number(weightText),exercise=exerciseText===''?0:Number(exerciseText),r=root.querySelector('#water-result');if(weightText===''||!Number.isFinite(weight)||weight<=0||!Number.isFinite(exercise)||exercise<0){r.innerHTML='<strong>입력값을 확인해 주세요</strong><p>몸무게는 0보다 크게, 운동 시간은 0분 이상으로 입력하세요.</p>';r.classList.add('show');(weightText===''||!Number.isFinite(weight)||weight<=0?root.querySelector('#water-weight'):root.querySelector('#water-exercise')).focus();return}const base=weight*30,extra=Math.ceil(exercise/30)*250;r.innerHTML=`<strong>${Math.round(base+extra).toLocaleString()} ml</strong><p>기본 ${Math.round(base).toLocaleString()}ml + 운동 추가 ${extra.toLocaleString()}ml 기준입니다.</p>`;r.classList.add('show')};
}
...
(function enhanceCategoryPages(){
  ...
  const safeLink=id=>calculators[id]?`<a href="${href(id)}"><b>${calculators[id].n}</b>${calculators[id].d?`<span>${calculators[id].d}</span>`:''}</a>`:'';
  const groupBlock=g=>{
  ...
  const data={
    money:{
      ...
      groups:[
        ...
        {title:'예금 / 적금',desc:'저축 만기 금액, 세후 이자, 월 납입 복리 투자 결과를 비교할 때 유용합니다.',ids:['savings-interest','installment','compound-interest','youth-leap-account','youth-account-switch']},
        {title:'월급 / 생활비',desc:'월 실수령액과 건강보험료, 가구 월평균소득, 생활 예산, 퇴직·실업 상황의 현금흐름을 점검하세요.',ids:['salary','monthly-average-income','employee-health-insurance','budget','unemployment-benefit','daily-proration','rent-conversion']},
    ...
    education:{
      ...
      groups:[
        ...
        {title:'시험 / 성적',desc:'평균 점수, 목표 점수, 내신 등급과 등급컷을 계산해 학습 계획에 활용하세요.',ids:['average-score','exam-target','school-grade','grade-cutoff']},
...
(function(){
  ...
  const added={
    ...
    'youth-account-switch':{n:'청년도약계좌 vs 청년미래적금 비교 계산기',c:'money',d:'청년도약계좌 유지와 청년미래적금 전환을 해지 유형, 기간, 원금, 정부기여금 기준으로 비교합니다.'},
...
function enhanceLifeDateTools(){
  ...
  if(key==='date'){
    root.innerHTML=`<a class="calculator-home" href="/">← 계산페이지 홈</a><h1>날짜 계산기</h1><p class="lead">기준일에서 며칠 전후 날짜를 계산하거나, 두 날짜 사이의 기간 차이를 확인합니다.</p><style>.date-tool-box{padding:0;overflow:hidden}.date-tool-head{padding:22px 24px 16px;background:#f8fbff;border-bottom:1px solid var(--line)}.date-tool-head h2{margin:0;font-size:20px;letter-spacing:-.5px}.date-tool-head p{margin:4px 0 0;color:#657181;font-size:14px}.date-mode-tabs{display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:16px 24px;background:#fff;border-bottom:1px solid var(--line)}.date-mode-tabs .primary-btn,.date-mode-tabs .secondary-btn{margin:0;min-height:48px;padding:11px 12px;border-radius:8px;font-weight:800}.date-mode-tabs .secondary-btn{border:1px solid #d9e4f5;background:#f8fafc;color:#334155}.date-tool-box .utility-form{padding:22px 24px;border-bottom:1px solid var(--line)}.date-tool-box .utility-form h2{margin:0 0 6px;font-size:19px}.date-panel-help{margin:0 0 16px;color:#657181;font-size:14px}.date-include-options{display:grid;grid-template-columns:1fr 1fr;gap:10px;align-self:end}.date-include-options .field-option{min-height:52px;margin:0;padding:12px;border:1px solid #d9e4f5;border-radius:8px;background:#fff;font-weight:800;color:#334155}.date-tool-box .result{margin:0 24px 18px}.date-tool-box .calculator-note{padding:0 24px 20px}@media(max-width:700px){.date-tool-head,.date-mode-tabs,.date-tool-box .utility-form{padding-left:16px;padding-right:16px}.date-mode-tabs{gap:8px}.date-mode-tabs .primary-btn,.date-mode-tabs .secondary-btn{font-size:13px;padding:10px 8px}.date-include-options{grid-template-columns:1fr}.date-tool-box .result{margin:0 16px 16px}.date-tool-box .calculator-note{padding:0 16px 18px}}</style><section class="calculator-box utility-box date-tool-box"><div class="date-tool-head"><h2>어떤 날짜 계산이 필요하세요?</h2><p>일정 앞뒤 날짜를 찾거나 두 날짜 사이 기간을 바로 비교할 수 있습니다.</p></div><div class="date-mode-tabs" role="tablist" aria-label="날짜 계산 방식"><button class="primary-btn" type="button" data-date-mode="add">며칠 전후 날짜</button><button class="secondary-btn" type="button" data-date-mode="diff">두 날짜 차이</button></div><div class="utility-form" data-date-panel="add"><h2>기준일에서 날짜 더하기·빼기</h2><p class="date-panel-help">기준일을 정하고 며칠 후 또는 며칠 전 날짜를 계산합니다.</p><div class="utility-fields"><label><span>기준일</span><input id="ld-date" type="date"></label><label><span>계산할 일수</span><input id="ld-days" type="number" placeholder="예: 100"></label><label><span>방향</span><select id="ld-direction"><option value="1">이후</option><option value="-1">이전</option></select></label></div><button class="primary-btn" id="ld-add-calc" type="button">전후 날짜 계산하기</button></div><div class="utility-form" data-date-panel="diff" hidden><h2>두 날짜 사이 차이 계산</h2><p class="date-panel-help">기본은 종료일에서 시작일을 뺀 값이며, 필요하면 시작일과 종료일을 각각 포함할 수 있습니다.</p><div class="utility-fields"><label><span>시작일</span><input id="ld-start" type="date"></label><label><span>종료일</span><input id="ld-end" type="date"></label><div class="date-include-options"><label class="field-option"><input id="ld-include-start" type="checkbox"> 시작일 포함</label><label class="field-option"><input id="ld-include-end" type="checkbox"> 종료일 포함</label></div></div><button class="primary-btn" id="ld-diff-calc" type="button">날짜 차이 계산하기</button></div><div class="result" id="ld-result" aria-live="polite"></div><p class="calculator-note">계약 기간, 준비 기간, 여행 일정처럼 날짜 기준이 중요한 경우 포함 여부에 따라 하루 차이가 날 수 있습니다.</p></section><section class="content-block"><h2>날짜 계산 기능</h2><ul><li><strong>며칠 전후 날짜</strong>: 기준일에서 100일 후, 30일 전처럼 특정 일수만큼 이동한 날짜를 계산합니다.</li><li><strong>두 날짜 차이</strong>: 시작일과 종료일 사이의 총 일수, 주 단위, 개월 환산값을 함께 확인합니다.</li></ul></section><section class="content-block"><h2>관련 계산기</h2><div class="related"><a href="/calculators/d-day.html">디데이 계산기</a><a href="/calculators/day-count.html">일수 계산기</a><a href="/calculators/age.html">나이 계산기</a><a href="/calculators/time.html">근무 시간 계산기</a></div></section>`;
    const parse=value=>value?new Date(`${value}T00:00:00`):null;
    const format=date=>date.toLocaleDateString('ko-KR',{year:'numeric',month:'long',day:'numeric',weekday:'short'});
    const daysBetween=(start,end)=>(Date.UTC(end.getFullYear(),end.getMonth(),end.getDate())-Date.UTC(start.getFullYear(),start.getMonth(),start.getDate()))/86400000;
    const show=html=>{const r=root.querySelector('#ld-result');r.innerHTML=html;r.classList.add('show')};
    root.querySelectorAll('[data-date-mode]').forEach(button=>button.onclick=()=>{
      const mode=button.dataset.dateMode;
      root.querySelectorAll('[data-date-mode]').forEach(item=>{item.classList.toggle('primary-btn',item.dataset.dateMode===mode);item.classList.toggle('secondary-btn',item.dataset.dateMode!==mode)});
      root.querySelectorAll('[data-date-panel]').forEach(panel=>panel.hidden=panel.dataset.datePanel!==mode);
      root.querySelector('#ld-result').classList.remove('show');
      root.querySelector('#ld-result').innerHTML='';
    });
    root.querySelector('#ld-add-calc').onclick=()=>{
      const base=parse(root.querySelector('#ld-date').value),days=Number(root.querySelector('#ld-days').value),dir=Number(root.querySelector('#ld-direction').value);
      if(!base||!Number.isFinite(days))return show('<strong>입력값을 확인해 주세요</strong><p>기준일과 계산할 일수를 입력하면 날짜를 계산할 수 있습니다.</p>');
      const resultDate=new Date(base);
      resultDate.setDate(resultDate.getDate()+days*dir);
      show(`<strong>${format(resultDate)}</strong><p>${format(base)} 기준 ${Math.abs(days).toLocaleString('ko-KR')}일 ${dir>0?'후':'전'} 날짜입니다.</p>`);
    };
    root.querySelector('#ld-diff-calc').onclick=()=>{
  ...
  if(key==='d-day'){
    ...
    root.querySelector('#ld-calc').onclick=()=>{
      ...
      const [baseYear,baseMonth,baseDay]=baseValue.split('-').map(Number),[targetYear,targetMonth,targetDay]=targetValue.split('-').map(Number);
      const raw=(Date.UTC(targetYear,targetMonth-1,targetDay)-Date.UTC(baseYear,baseMonth-1,baseDay))/86400000;
      if(!Number.isFinite(raw)){result.innerHTML='<strong>날짜를 확인해 주세요</strong><p>유효한 기준일과 목표일을 선택해 주세요.</p>';result.classList.add('show');return}
      const includeBase=root.querySelector('#ld-include-base').checked,includeTarget=root.querySelector('#ld-include-target').checked;
      const days=Math.max(0,Math.abs(raw)+(includeBase?1:0)-(includeTarget?0:1)),weeks=Math.floor(days/7),remain=days%7;
      const label=raw===0?'D-Day':raw>0?`D-${days}`:`D+${days}`;
      const direction=raw===0?'기준일과 목표일이 같습니다.':raw>0?`목표일까지 ${days}일 남았습니다.`:`목표일로부터 ${days}일 지났습니다.`;
      result.innerHTML=`<div class="utility-result-grid"><div><span>디데이</span><strong>${label}</strong></div><div><span>주 단위</span><b>${weeks}주 ${remain}일</b></div><div><span>날짜 차이</span><b>${days.toLocaleString('ko-KR')}일</b></div></div><p>${direction} 기준일 ${includeBase?'포함':'제외'}, 목표일 ${includeTarget?'포함':'제외'} 기준입니다.</p>`;
      result.classList.add('show');
    };
    return;
  }
  if(key==='time')root.innerHTML=`<a class="calculator-home" href="/">← 계산페이지 홈</a><h1>근무 시간 계산기</h1><p class="lead">자정 넘김과 휴게시간을 반영해 실제 경과 시간을 계산합니다.</p><section class="calculator-box utility-box"><div class="utility-form"><div class="utility-fields"><label>시작 시간<input id="lt-start" type="time"></label><label>종료 시간<input id="lt-end" type="time"></label><label>제외할 시간(분)<input id="lt-break" type="number" placeholder="예: 60"></label></div><button class="primary-btn" id="lt-calc">시간 계산하기</button></div><div class="result" id="lt-result"></div></section>`;
const button=root.querySelector('.primary-btn');button.onclick=()=>{const r=root.querySelector('.result');if(key==='time'){let a=root.querySelector('#lt-start').value,b=root.querySelector('#lt-end').value,brText=root.querySelector('#lt-break').value.trim(),br=brText===''?0:Number(brText);if(!a||!b||!Number.isFinite(br)||br<0){r.innerHTML='<strong>입력값을 확인해 주세요</strong><p>시작·종료 시간과 0분 이상의 제외 시간을 입력하세요.</p>';r.classList.add('show');(!a?root.querySelector('#lt-start'):!b?root.querySelector('#lt-end'):root.querySelector('#lt-break')).focus();return}let m=t=>t.split(':').reduce((s,v)=>s*60+(+v),0),d=m(b)-m(a);if(d<0)d+=1440;if(br>d){r.innerHTML='<strong>제외 시간을 확인해 주세요</strong><p>제외 시간은 전체 경과 시간보다 클 수 없습니다.</p>';r.classList.add('show');root.querySelector('#lt-break').focus();return}d-=br;r.innerHTML=`<strong>${Math.floor(d/60)}시간 ${d%60}분</strong><p>휴게시간 ${br}분을 제외한 시간입니다.</p>`}else{const a=new Date(root.querySelector('#ld-date').value);if(isNaN(a))return;if(key==='date'){const days=+root.querySelector('#ld-days').value,dir=+root.querySelector('#ld-direction').value;if(!Number.isFinite(days))return;a.setDate(a.getDate()+days*dir);r.innerHTML=`<strong>${a.toLocaleDateString('ko-KR')}</strong>`}else{const b=new Date(root.querySelector('#ld-target').value);if(isNaN(b))return;let d=Math.round((b-a)/864e5);if(root.querySelector('#ld-include').checked)d+=d>=0?1:-1;r.innerHTML=`<strong>${d>=0?'D-'+d:'D+'+Math.abs(d)}</strong><p>기준일과 목표일의 차이입니다.</p>`}}r.classList.add('show')};
}
...
root.querySelector('#ch-calc').onclick=()=>{const r=root.querySelector('#ch-result');let out,text;if(key==='bmi'){const h=+root.querySelector('#ch0').value,w=+root.querySelector('#ch1').value;if(!h||!w){r.innerHTML='<strong>키와 몸무게를 입력해 주세요</strong>';r.classList.add('show');return}const n=w/(h/100)**2;out=`${n.toFixed(1)} BMI`;text=n<18.5?'저체중 범위':n<23?'정상 범위':n<25?'과체중 범위':'비만 범위'}if(key==='bmr'){const sex=root.querySelector('#ch0').value,age=+root.querySelector('#ch1').value,h=+root.querySelector('#ch2').value,w=+root.querySelector('#ch3').value;if(!age||!h||!w){r.innerHTML='<strong>나이, 키, 몸무게를 입력해 주세요</strong><p>모든 값은 0보다 크게 입력하세요.</p>';r.classList.add('show');(!age?root.querySelector('#ch1'):!h?root.querySelector('#ch2'):root.querySelector('#ch3')).focus();return}const n=sex==='male'?10*w+6.25*h-5*age+5:10*w+6.25*h-5*age-161;out=`${Math.round(n)} kcal`;text='하루 기초대사량 추정치'}if(key==='target-weight'){const h=+root.querySelector('#ch0').value,b=+root.querySelector('#ch1').value;if(!h||!b){r.innerHTML='<strong>키와 목표 BMI를 입력해 주세요</strong><p>두 값은 0보다 크게 입력하세요.</p>';r.classList.add('show');(!h?root.querySelector('#ch0'):root.querySelector('#ch1')).focus();return}out=`${((h/100)**2*b).toFixed(1)} kg`;text='목표 BMI 기준 체중'}if(key==='running-pace'){const d=+root.querySelector('#ch0').value,m=+root.querySelector('#ch1').value||0,s=+root.querySelector('#ch2').value||0,total=m*60+s;if(!d||total<=0){r.innerHTML='<strong>거리와 시간을 입력해 주세요</strong>';r.classList.add('show');return}const perKm=Math.round(total/d);out=`${Math.floor(perKm/60)}분 ${String(perKm%60).padStart(2,'0')}초 /km`;text='평균 러닝 페이스'}r.innerHTML=`<strong>${out}</strong><p>${text}</p>`;r.classList.add('show')};
}
...
if(document.body.dataset.customCalculator==='volumetric-weight'){
  ...
  const divisor=root.querySelector('#vw-divisor'),custom=root.querySelector('#vw-custom');custom.setAttribute('aria-label','직접 입력할 부피무게 분모');custom.min='1';divisor.onchange=()=>custom.hidden=divisor.value!=='custom';
...
const advancedSearchTools={
  dsr:['DSR 계산기','money','연소득 대비 기존·신규 대출의 연간 원리금 상환 비율을 계산합니다.'],
  'mortgage-loan':['주택담보대출 계산기','money','주택가격·대출액·금리·상환 방식으로 월 납입액과 LTV를 추정합니다.'],
  'monthly-average-income':['월평균합산소득 계산기','money','가구원별 세전 소득을 월 단위로 합산해 공공주택 소득기준과 비교합니다.'],
  'stock-return':['주식 수익률 계산기','money','매수·매도 단가와 수량·왕복 수수료로 손익과 수익률을 계산합니다.'],
  'prepayment-fee':['중도상환수수료 계산기','money','상환 원금·수수료율·잔존기간으로 예상 중도상환수수료를 계산합니다.'],
  'car-acquisition-tax':['자동차 취득세 계산기','money','차량 과세표준과 적용 세율로 예상 취득세를 계산합니다.'],
  'card-installment':['카드 할부 수수료 계산기','money','결제 금액·할부 기간·수수료율로 월 부담액과 총 수수료를 계산합니다.'],
  'housing-subscription':['청약 가점 계산기','money','무주택기간·부양가족·통장 가입기간으로 민영주택 청약 가점을 계산합니다.'],
  'monthly-rent-deduction':['월세 세액공제 계산기','money','총급여·월세·거주기간으로 연말정산 월세 세액공제 예상액을 계산합니다.'],
  'loan-schedule':['대출 상환 스케줄 계산기','money','상환 방식별 월 원금·이자·잔액의 흐름을 표로 계산합니다.'],
  'employee-health-insurance':['직장인 건강보험료 계산기','money','월 보수액으로 건강보험·장기요양보험 근로자 부담분을 계산합니다.'],
  'grade-cutoff':['등급컷 계산기','education','수강 인원과 5·9등급 누적 비율로 예상 석차 경계를 계산합니다.'],
  ovulation:['배란일·가임기 계산기','health','최근 생리 시작일과 평균 주기로 예상 배란일과 가임기를 계산합니다.'],
  'menstrual-cycle':['생리 주기 계산기','health','최근 생리 시작일과 평균 주기로 다음 시작일을 추정합니다.']
};
Object.entries(advancedSearchTools).forEach(([key,value])=>calculators[key]={n:value[0],c:value[1],d:value[2]});
cats.money[3]+=' dsr mortgage-loan monthly-average-income stock-return prepayment-fee car-acquisition-tax card-installment housing-subscription monthly-rent-deduction loan-schedule employee-health-insurance';
cats.health[3]+=' ovulation menstrual-cycle';
cats.education[3]+=' grade-cutoff';
if(document.querySelector('.popular-list')){home();improveCalculatorSearch();}
if(document.body.dataset.category)category();
if(document.querySelector('#category-grid'))makeHomeCategoriesExpandable();
...
(function(){
  if(typeof calculators==='undefined'||typeof cats==='undefined')return;
  calculators['mortgage-loan']={n:'주택담보대출 계산기',c:'money',d:'주택가격, 대출희망액, 금리, 기간, 상환방식으로 주담대 월 상환액과 LTV, 첫해 예상 DSR을 계산합니다.'};
  calculators['monthly-average-income']={n:'월평균합산소득 계산기',c:'money',d:'청약·임대주택 신청 전 본인·부모·배우자 소득을 월평균으로 합산하고 도시근로자 소득기준과 비교합니다.'};
  calculators.cagr={n:'CAGR 계산기',c:'money',d:'시작 금액, 종료 금액, 기간으로 연평균 성장률과 총 수익률을 계산합니다.'};
  if(calculators['employee-health-insurance']){
    calculators['employee-health-insurance'].d='월 보수액으로 직장가입자 건강보험료와 장기요양보험료 근로자 부담분을 계산합니다.';
  }
  cats.money[3]+=' mortgage-loan monthly-average-income cagr';
  if(typeof searchAliases!=='undefined')Object.assign(searchAliases,{cagr:'cagr 연평균 성장률 연평균수익률 복리수익률 투자수익률 성장률'});
...
(function(){
  ...
  const descriptions={
    ...
    'compound-interest':'초기 원금과 월 추가 납입액을 기준으로 적립식 복리 투자 결과와 연도별 잔액을 확인합니다.',
    'loan-interest':'대출 원금, 금리, 기간을 입력해 월 상환액, 총 이자, 전체 상환 부담을 추정합니다.',
    'mortgage-loan':'주택가격, 대출희망액, 금리, 기간, 상환방식으로 주담대 월 상환액과 LTV, 첫해 예상 DSR을 계산합니다.',
    'monthly-average-income':'청약·임대주택 신청 전 본인·부모·배우자 소득을 월평균으로 합산하고 도시근로자 소득기준과 비교합니다.',
    ...
    'exercise-calorie':'몸무게, 운동 종류, 시간을 입력해 예상 운동 소모 칼로리를 계산합니다.',
    'target-weight':'키, 현재 체중, 목표 BMI를 기준으로 목표 체중과 현재 체중의 차이를 계산합니다.',
    ...
    margin:'판매가, 원가, 수수료, 배송·광고비와 부가세 기준을 반영해 실질 마진율을 계산합니다.',
    ...
    'work-hours':'7일간 근무 기록의 날짜별 합계, 휴게시간과 CSV 내역을 확인합니다.',
...
(function(){
  ...
  if(typeof searchBoostIds!=='undefined'){
    ['loan-refinance','ev-fuel-cost'].forEach(id=>{if(!searchBoostIds.includes(id))searchBoostIds.push(id)});
  }
  if(document.querySelector('.popular-list')){
    home();
    if(typeof improveCalculatorSearch==='function')improveCalculatorSearch();
