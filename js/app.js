...
function category(){let key=document.body.dataset.category;if(!key)return;let c=cats[key];document.title=`${c[0]} | 계산페이지`;document.querySelector('#category').innerHTML=`<p class="eyebrow">${c[2]}</p><h1>${c[0]}</h1><p class="lead">${c[0]}에 필요한 무료 계산기를 모았습니다. 원하는 항목을 선택해 바로 계산하세요.</p><div class="card-grid">${c[3].split(' ').map(card).join('')}</div>`}home();category();page();
