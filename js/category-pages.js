(function(){
  ...
  const data={
    money:{
      eyebrow:'대출·저축·세금·투자',
      title:'금융 계산기',
      lead:'대출 이자와 갈아타기, 예금 이자, 월급, 구독료, 세금, 투자 계산까지 생활에 필요한 금융 계산기를 한곳에 모았습니다.',
      ...
      comparisons:[['예금과 적금','예금은 목돈을 한 번 맡기고, 적금은 매달 나눠 납입합니다. 같은 금리라도 돈이 맡겨지는 기간이 달라 이자가 다릅니다.'],['대출 이자와 갈아타기','대출 이자 계산기는 한 대출의 상환 부담을 보고, 갈아타기 계산기는 현재·새 대출의 이자와 수수료를 함께 비교합니다.'],['대출 이자와 상환 스케줄','대출 이자 계산기는 상환 방식별 총 부담을 비교하고, 상환 스케줄은 매월 원금·이자·잔액의 흐름을 회차별 표로 확인할 때 적합합니다.'],['원리금균등과 원금균등','원리금균등은 월 납입액이 거의 일정하고, 원금균등은 첫 달 부담이 크지만 시간이 갈수록 줄어 일반적으로 총이자가 적습니다.'],['ROI와 CAGR','ROI는 전체 투자기간의 수익률이고, CAGR은 시작값과 종료값을 기간에 맞춰 연평균 복리 성장률로 환산합니다.'],['퍼센트와 할인율','퍼센트 계산기는 비율 자체를 구하고, 할인율 계산기는 정가에서 줄어드는 금액과 최종 가격을 구합니다.']],
    ...
    health:{
      ...
      comparisons:[['BMI와 목표 체중','BMI는 현재 키·몸무게의 지수를 구하고, 목표 체중은 선택한 목표 BMI에 해당하는 체중을 역산합니다.'],['기초대사량과 권장 칼로리','기초대사량은 안정 상태 소비량 추정치이고, 권장 칼로리는 활동량을 더해 하루 필요량을 봅니다.'],['러닝 페이스와 속도','러닝 페이스는 1km에 걸린 시간, 속도는 1시간에 이동한 거리이므로 숫자가 클수록 빠르다는 방향도 서로 다릅니다.']],
    ...
    life:{
      ...
      title:'생활 계산기',
      lead:'날짜, 시간, 나이, 더치페이, 차량 유지비, 단위 변환, 배송·부피 계산 등 일상에서 자주 쓰는 계산기를 모았습니다.',
      recommend:['d-day','date','age','dutch-pay','unit'],
      groups:[{title:'날짜 / 시간',desc:'기준일, 디데이, 일수, 경과 시간을 빠르게 확인합니다.',ids:['date','d-day','day-count','time']},{title:'나이 / 생일',desc:'한국식 나이와 만나이를 함께 비교해 서류나 일정 확인에 활용하세요.',ids:['age','international-age']},{title:'비용 나누기 / 이동',desc:'더치페이, 여행 예산, 주유비와 전기차 유지비처럼 일상 비용을 비교합니다.',ids:['dutch-pay','travel-budget','fuel-cost','ev-fuel-cost']},{title:'면적 / 변환 / 물류',desc:'평수, 단위, 스케일, CBM, 부피무게처럼 규격 계산이 필요할 때 사용합니다.',ids:['area-conversion','unit','scale','cbm','volumetric-weight']},{title:'생활 공과금',desc:'전기요금처럼 월별 생활비를 추정할 때 참고하세요.',ids:['electricity','car-tax']}],
      comparisons:[['날짜·디데이·일수','날짜 계산은 며칠 전후의 날짜, 디데이는 목표일까지 남은 기간, 일수 계산은 두 날짜 사이 기간을 구합니다.'],['주유비와 전기차 유지비','주유비는 한 차량의 이동 비용을 계산하고, 전기차 유지비는 같은 주행거리에서 내연기관 유류비와 전기차 충전비를 비교합니다.'],['주유비와 여행 경비','주유비는 차량 이동비를 거리·연비로 계산하고, 여행 경비는 교통·숙박·식비 전체 예산을 합산합니다.'],['더치페이와 배송비 분할','더치페이는 사람별 균등 분담에 적합하고, 배송비 분할은 상품 금액이나 수량 기준 배분을 확인할 때 적합합니다.']],
  ...
  root.innerHTML=`
    ...
    <section class="content-block category-comparison"><h2>계산기 선택 가이드</h2><div class="category-purpose-grid">${data.comparisons.map(item=>`<article class="category-purpose-card"><h3>${item[0]}</h3><p>${item[1]}</p></article>`).join('')}</div></section>
...
(function(){
  ...
  root.innerHTML=`
    ...
    <section class="content-block category-guide conversion-rich-guide">
      <h2>단위환산 계산기를 사용할 때 알아두세요</h2>
      <p>단위환산은 숫자 하나를 다른 단위로 바꾸는 단순한 작업처럼 보이지만, 실제로는 사용하는 분야에 따라 확인해야 할 기준이 조금씩 다릅니다. 길이와 무게처럼 국제 단위가 비교적 명확한 항목도 있고, 평과 제곱미터처럼 생활 속 관행 단위가 함께 쓰이는 항목도 있습니다. 이 페이지는 자주 쓰는 변환 계산기를 한곳에 모아 두어, 필요한 단위를 찾고 같은 기준으로 다시 확인할 수 있게 만든 카테고리입니다.</p>
      <p>계산 결과는 표준 환산 계수를 기준으로 보여줍니다. 다만 업체, 학교, 기관, 물류사마다 반올림 자리, 표시 단위, 최소 청구 기준이 다를 수 있습니다. 공식 서류 제출, 시험 답안 작성, 계약서 확인, 배송비 정산처럼 금액이나 제출 결과에 영향을 주는 상황이라면 이 계산기로 먼저 감을 잡고 최종 기준은 해당 기관의 안내를 함께 확인하는 것이 좋습니다.</p>
      <div class="category-purpose-grid">
        <article class="category-purpose-card"><h3>생활에서 자주 쓰는 변환</h3><p>해외 상품 사이즈, 여행 거리, 조리 온도, 체중·중량 표기처럼 익숙하지 않은 단위를 빠르게 바꿔 볼 때 유용합니다. 같은 값을 여러 단위로 바꿔 보면 숫자의 크기를 더 쉽게 이해할 수 있습니다.</p></article>
        <article class="category-purpose-card"><h3>부동산·인테리어 단위</h3><p>제곱미터, 평, 축척, 실제 길이처럼 공간을 비교해야 하는 상황에서 도움이 됩니다. 전용면적과 공급면적처럼 기준이 다른 값은 같은 단위로 바꾼 뒤 의미를 구분해서 보는 편이 안전합니다.</p></article>
        <article class="category-purpose-card"><h3>물류·배송 단위</h3><p>박스 크기, CBM, 부피무게는 배송비와 연결되기 쉽습니다. 단위 변환 결과와 실제 운송사 기준이 항상 같지는 않으므로, 예상 비용을 보기 전에는 반올림 방식과 최소 청구 중량을 함께 확인해 주세요.</p></article>
        <article class="category-purpose-card"><h3>공부·업무용 확인</h3><p>과제, 시험, 보고서, 견적서에 단위를 적을 때는 값보다 표기 기준이 더 중요할 때가 있습니다. 소수점 자리와 단위 기호를 맞춰 두면 나중에 결과를 다시 확인할 때 혼동을 줄일 수 있습니다.</p></article>
      </div>
    </section>
    <section class="content-block category-comparison"><h2>단위환산 계산기 선택 가이드</h2><div class="category-purpose-grid"><article class="category-purpose-card"><h3>범용 단위와 평수 계산</h3><p>범용 변환은 같은 물리량의 단위를 바꾸고, 평수 계산은 ㎡·평 환산과 전용률처럼 부동산 면적의 의미까지 함께 확인합니다.</p></article><article class="category-purpose-card"><h3>CBM과 부피무게</h3><p>CBM은 화물의 공간 부피이고, 부피무게는 운송사 분모를 적용해 실제 중량과 비교하는 배송비 산정용 값입니다.</p></article><article class="category-purpose-card"><h3>스케일과 길이 변환</h3><p>길이 변환은 단위만 바꾸지만, 스케일 계산은 축척 비율을 적용해 도면 길이와 실제 길이를 오갑니다.</p></article></div></section>
