...
(function(){
  ...
  const show=(id,html)=>{const r=root.querySelector('#'+id);r.innerHTML=html;r.classList.add('show');};
  ...
  const loanRows=(principal,annualRate,months,method,limit=12)=>{
    ...
    for(let i=1;i<=count;i++){
      let principalPart=0,interest=balance*r,payment=0;
      if(method==='annuity'){payment=fixed;principalPart=payment-interest;balance=Math.max(0,balance-principalPart);}
      else if(method==='principal'){principalPart=principal/months;payment=principalPart+interest;balance=Math.max(0,balance-principalPart);}
      else{payment=interest;principalPart=0;if(i===months){payment+=principal;principalPart=principal;balance=0;}}
  ...
  if(slug==='salary'){
    renderShell('월급 실수령액 계산기','월 세전 급여와 비과세액만 입력하면 4대보험, 소득세, 지방소득세를 나눠 예상 실수령액을 계산합니다.',
      ...
      '<section class="content-block"><h2>계산에 포함한 항목</h2><p>국민연금, 건강보험, 장기요양보험, 고용보험, 간편 소득세, 지방소득세를 각각 계산해 공제 내역으로 보여줍니다.</p></section>');
    root.querySelector('#ep-calc').onclick=()=>{
      const gross=num('ep-gross'), nontax=num('ep-nontax'), year=val('ep-year');
      ...
      const monthlyIncomeTax=tax.income/12, monthlyLocal=tax.local/12;
  ...
  if(slug==='four-insurance'){
    renderShell('4대보험 계산기','월 급여와 비과세액을 입력해 국민연금, 건강보험, 장기요양보험, 고용보험을 근로자 부담과 회사 부담으로 나눠 계산합니다.',
    ...
    root.querySelector('#ei-calc').onclick=()=>{
      const gross=num('ei-gross'), year=val('ei-year');
      if(!gross)return invalid('ei-result','월 세전 급여를 입력해 주세요.');
      const ins=calcInsurance(gross,num('ei-nontax'),year);
      const employer=val('ei-employer')==='yes';
      const employerHtml=employer?`${card('회사 부담 추정',money(ins.total),'산재보험 제외')}`:'';
  ...
  if(slug==='income-tax'){
    renderShell('근로소득세 계산기','연간 총급여, 예상 공제, 세액공제, 이미 낸 세금을 입력해 소득세와 지방소득세 예상액을 계산합니다.',
      ...
      '<section class="content-block"><h2>계산 방식</h2><p>과세표준 = 연간 총급여 - 입력한 공제액으로 보고, 누진세율과 누진공제를 적용한 뒤 세액공제와 이미 낸 세금을 차감합니다.</p></section>');
  ...
  if(slug==='loan-interest'||slug==='jeonse-loan'){
    ...
    renderShell(isJeonse?'전세대출 이자 계산기':'대출 이자 계산기',isJeonse?'전세대출 금액, 금리, 기간, 상환 방식을 입력해 월 부담액과 총 이자를 계산합니다.':'대출 원금, 금리, 기간, 상환 방식을 입력해 월 상환액과 총 이자, 월별 상환표를 확인합니다.',
      `<section class="calculator-box utility-box loan-box"><div class="utility-form"><div class="utility-fields">${isJeonse?field('el-deposit','전세보증금(원)','300000000'):''}${field('el-principal',isJeonse?'전세대출금(원)':'대출 원금(원)',isJeonse?'200000000':'100000000')}${field('el-rate','연 이자율(%)','4.5')}${field('el-months','상환 기간(개월)','36','1')}${isJeonse?field('el-income','연소득(원, 선택)','50000000'):''}<label><span>상환 방식</span><select id="el-method"><option value="annuity">원리금균등상환</option><option value="principal">원금균등상환</option><option value="bullet">${isJeonse?'만기일시상환(이자만 납부)':'만기일시상환'}</option></select></label></div><button class="primary-btn" id="el-calc" type="button">대출 계산하기</button></div><div class="result" id="el-result" aria-live="polite"></div></section>`,
...
(function(){
  ...
  if(slug==='housing-subscription'){
    shell(
      '청약 가점 계산기',
      '무주택기간, 부양가족 수, 청약통장 가입기간을 실제 가점표 구간으로 선택해 총 84점 만점 기준 예상 청약 가점을 계산합니다.',
      select('hs-house','무주택기간 구간',[
        ['0','0점 · 30세 미만 미혼 또는 무주택기간 없음'],
        ['2','2점 · 1년 미만'],
        ['4','4점 · 1년 이상 ~ 2년 미만'],
        ['6','6점 · 2년 이상 ~ 3년 미만'],
        ['8','8점 · 3년 이상 ~ 4년 미만'],
        ...
        ['18','18점 · 8년 이상 ~ 9년 미만'],
        ['20','20점 · 9년 이상 ~ 10년 미만'],
        ['22','22점 · 10년 이상 ~ 11년 미만'],
        ['24','24점 · 11년 이상 ~ 12년 미만'],
        ['26','26점 · 12년 이상 ~ 13년 미만'],
        ['28','28점 · 13년 이상 ~ 14년 미만'],
        ['30','30점 · 14년 이상 ~ 15년 미만'],
        ['32','32점 · 15년 이상']
      ]) +
      select('hs-family','부양가족 수',[
        ['5','0명 · 5점'],
        ['10','1명 · 10점'],
        ['15','2명 · 15점'],
        ['20','3명 · 20점'],
      ...
      select('hs-account','청약통장 가입기간',[
        ...
        ['10','10점 · 8년 이상 ~ 9년 미만'],
        ['11','11점 · 9년 이상 ~ 10년 미만'],
        ['12','12점 · 10년 이상 ~ 11년 미만'],
        ['13','13점 · 11년 이상 ~ 12년 미만'],
        ['14','14점 · 12년 이상 ~ 13년 미만'],
        ['15','15점 · 13년 이상 ~ 14년 미만'],
        ['16','16점 · 14년 이상 ~ 15년 미만'],
        ['17','17점 · 15년 이상']
      ]),
      '청약 가점은 민영주택 일반공급 가점제 이해를 돕는 참고용입니다. 무주택 인정, 부양가족 인정, 모집공고별 가점제 적용 비율과 지역 조건은 반드시 입주자모집공고와 청약홈 기준을 확인하세요.',
      '<h2>청약 가점제란?</h2><p>청약 가점제는 동일 순위 안에서 경쟁이 있을 때 무주택기간, 부양가족 수, 청약통장 가입기간을 점수로 환산해 높은 점수 순으로 당첨자를 선정하는 방식입니다. 세 항목의 만점은 각각 32점, 35점, 17점이며 총점은 84점입니다.</p><table class="rate-table"><thead><tr><th>항목</th><th>만점</th><th>핵심 기준</th></tr></thead><tbody><tr><td>무주택기간</td><td>32점</td><td>일반적으로 만 30세가 되는 날부터 산정하며, 30세 이전 혼인 시 혼인신고일부터 볼 수 있습니다.</td></tr><tr><td>부양가족 수</td><td>35점</td><td>본인은 제외하고 배우자, 직계존속, 직계비속 등 인정 요건을 충족하는 가족을 기준으로 봅니다.</td></tr><tr><td>청약통장 가입기간</td><td>17점</td><td>입주자모집공고일 현재 가입기간을 기준으로 하며, 최초 가입일 기준으로 산정하는 것이 일반적입니다.</td></tr></tbody></table><h2>입력 전 꼭 확인하세요</h2><ul><li>무주택기간은 본인과 배우자의 주택 소유 이력에 따라 달라질 수 있습니다.</li><li>부양가족 수에는 청약 신청자 본인을 포함하지 않습니다.</li><li>직계존속은 일정 기간 같은 주민등록표에 등재되어야 하는 등 세부 인정 요건이 있습니다.</li><li>가점이 높아도 지역, 주택형, 경쟁률, 특별공급 여부에 따라 당첨 가능성은 달라집니다.</li></ul>'
    );
    root.querySelector('#calc2').onclick = () => {
      const houseScore=num('hs-house'), familyScore=num('hs-family'), accountScore=num('hs-account');
      const total=houseScore+familyScore+accountScore;
      const level=total>=70?'매우 높은 가점권':total>=60?'높은 가점권':total>=45?'중간 가점권':'낮은 가점권';
      const selected=id=>root.querySelector('#'+id).selectedOptions[0].textContent.replace(/^\d+점 · /,'');
      show(`<div class="savings-result-grid">${card('예상 청약 가점',total+'점 / 84점')}${card('무주택기간 점수',houseScore+'점')}${card('부양가족 점수',familyScore+'점')}${card('통장 가입기간 점수',accountScore+'점')}</div><p><strong>${level}</strong>입니다. 다만 실제 당첨 가능성은 지역, 주택형, 경쟁률, 가점 커트라인에 따라 달라집니다.</p><table class="rate-table"><thead><tr><th>항목</th><th>선택한 구간</th><th>점수</th></tr></thead><tbody><tr><td>무주택기간</td><td>${selected('hs-house')}</td><td>${houseScore}점 / 32점</td></tr><tr><td>부양가족 수</td><td>${selected('hs-family')}</td><td>${familyScore}점 / 35점</td></tr><tr><td>청약통장 가입기간</td><td>${selected('hs-account')}</td><td>${accountScore}점 / 17점</td></tr></tbody></table><table class="rate-table"><thead><tr><th>총점 구간</th><th>해석</th></tr></thead><tbody><tr><td>70점 이상</td><td>가점이 매우 높은 편이지만 인기 지역은 커트라인이 더 높을 수 있습니다.</td></tr><tr><td>60점대</td><td>지역과 주택형에 따라 경쟁력이 있을 수 있어 최근 당첨 가점을 함께 확인하는 것이 좋습니다.</td></tr><tr><td>45~59점</td><td>가점제 물량만 보기보다 추첨제, 특별공급, 지역 조건도 함께 검토해 보세요.</td></tr><tr><td>44점 이하</td><td>무주택기간, 통장 가입기간은 시간이 지나며 올라갈 수 있으므로 장기 계획을 세우는 데 활용하세요.</td></tr></tbody></table>`);
    };
  }
})();
